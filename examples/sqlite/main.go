...
func main() {
	...
	// if the file temporal.db exists, delete it
	if err := os.Remove("temporal.db"); err != nil && !os.IsNotExist(err) {
		log.Fatalf("os.Remove() failed: %v", err)
	}
	...
	sql.Register("sqlite-cached", interceptor.Driver(db.Driver()))
...
func initDb() (*sql.DB, error) {
	...
	db, err := sql.Open("sqlite", "temporal.db")
	if err != nil {
		return nil, err
	}
	...
	// insert some data
	_, err = db.Exec(`INSERT INTO data (user, timestamp, value) VALUES
...
func run() error {
	...
	db, err := sql.Open("sqlite-cached", "temporal.db")
	if err != nil {
		return err
	}
	defer db.Close()
...
func doQuery(db *sql.DB) error {
	...
	rows, err := db.QueryContext(context.TODO(), `
		-- @cache-ttl 30
		-- @cache-max-rows 1000
		SELECT user, timestamp FROM data WHERE value > $1`, 80)
	if err != nil {
